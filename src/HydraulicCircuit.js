...
class HydraulicCircuit extends React.Component {
  ...
  click = (obj) => {
    if(!Object.keys(this.props.plc_vars).includes("Y1"))
      return;
    if (obj.id === "Y1_") {
      let upd_vars = [...[this.props.plc_vars["Y1"]]];
      upd_vars.forEach((el) => (el.value = false));
      this.props.handler(upd_vars);
    } else if (obj.id === "Y2_") {
      let upd_vars = [...[this.props.plc_vars["Y2"]]];
      upd_vars.forEach((el) => (el.value = false));
      this.props.handler(upd_vars);
    } else if (obj.id === "Y3_") {
      let upd_vars = [...[this.props.plc_vars["Y3"]]];
      upd_vars.forEach((el) => (el.value = false));
      this.props.handler(upd_vars);
    } else if (obj.id === "Y6_") {
      let upd_vars = [...[this.props.plc_vars["Y6"]]];
      upd_vars.forEach((el) => (el.value = false));
      this.props.handler(upd_vars);
    } else if (obj.id === "Y7_") {
      let upd_vars = [...[this.props.plc_vars["Y7"]]];
      upd_vars.forEach((el) => (el.value = false));
      this.props.handler(upd_vars);
    } else if (obj.id === "Y18_") {
      let upd_vars = [...[this.props.plc_vars["Y18"]]];
      upd_vars.forEach((el) => (el.value = false));
      this.props.handler(upd_vars);
    } else if (obj.id === "Y4_5") {
      let upd_vars = [
        ...[this.props.plc_vars["Y4"], this.props.plc_vars["Y5"]],
      ];
      upd_vars.forEach((el) => (el.value = false));
      this.props.handler(upd_vars);
    } else if (obj.id === "Y16_17") {
      let upd_vars = [
        ...[this.props.plc_vars["Y16"], this.props.plc_vars["Y17"]],
      ];
      upd_vars.forEach((el) => (el.value = false));
      this.props.handler(upd_vars);
    } else if (obj.id === "Y8_9") {
      let upd_vars = [
        ...[this.props.plc_vars["Y8"], this.props.plc_vars["Y9"]],
      ];
      upd_vars.forEach((el) => (el.value = false));
      this.props.handler(upd_vars);
    } else if (obj.id === "Y10_11") {
      let upd_vars = [
        ...[this.props.plc_vars["Y10"], this.props.plc_vars["Y11"]],
      ];
      upd_vars.forEach((el) => (el.value = false));
      this.props.handler(upd_vars);
    } else if (obj.id === "Y12_13") {
      let upd_vars = [
        ...[this.props.plc_vars["Y12"], this.props.plc_vars["Y13"]],
      ];
      upd_vars.forEach((el) => (el.value = false));
      this.props.handler(upd_vars);
    } else if (obj.id === "Y14_15") {
      let upd_vars = [
        ...[this.props.plc_vars["Y14"], this.props.plc_vars["Y15"]],
      ];
      upd_vars.forEach((el) => (el.value = false));
      this.props.handler(upd_vars);
    } else {
      if (this.props.plc_vars[obj.id] !== undefined) {
        ...
        let upd_vars = [...[this.props.plc_vars[obj.id]]];
    ...
    return (
      <div>
        <MemoHydraulicCirctuit
          ...
          Y10_11={
            !this.props.plc_vars.Y10?.value && !this.props.plc_vars.Y11?.value
          }
          Y12={this.props.plc_vars.Y12?.value}
          Y13={this.props.plc_vars.Y13?.value}
          Y12_13={
            !this.props.plc_vars.Y12?.value && !this.props.plc_vars.Y13?.value
          }
          Y14={this.props.plc_vars.Y14?.value}
          Y15={this.props.plc_vars.Y15?.value}
          Y14_15={
            !this.props.plc_vars.Y14?.value && !this.props.plc_vars.Y15?.value
          }
          Y16={this.props.plc_vars.Y16?.value}
          Y17={this.props.plc_vars.Y17?.value}
          Y16_17={
            !this.props.plc_vars.Y16?.value && !this.props.plc_vars.Y17?.value
          }
